...
public class UserDAO {
    ...
    public boolean addUser(String username, String password, String email) {
    	...
    	long count = usersCollection.count(Filters.eq("_id", username));
    	if( count > 0 ){
    		System.out.println("user with username: " + username + " already exits");
    		return false;
    	}
    ...
    public Document validateLogin(String username, String password) {
        ...
        List<Document> all = usersCollection.find(Filters.eq("_id", username)).into(new ArrayList<Document>());
        if( all.size() > 0 ) for (Document d : all ){
