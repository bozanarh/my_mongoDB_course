...
public class BlogPostDAO {
    ...
    public void likePost(final String permalink, final int ordinal) {
    	...
    	//I am interested in comments and permalink only
    	Document projection = new Document().append("comments", 1).append("permalink", 1);
		
    	//Find the document of interest (the one that matches permalink)
    	List<Document> xDocs = postsCollection.find(filter)
    				.projection(projection)
    				.into(new ArrayList<Document>());

    	if( xDocs.size() == 1 ){
    		/*
    		//This is for debugging purpose only - I want to print how many likes it has
			...
			*/
    		//Now, inc num_likes and that is it!
			StringBuilder likeNum = new StringBuilder();
			//Note this is how you access array element, e.g. "comment.5.num_likes" will update 6th elem!
			likeNum.append("comments.").append(ordinal).append(".num_likes");
			postsCollection.updateOne(filter, new Document("$inc", new Document(likeNum.toString(), 1)));
    	}    	
    }
}
